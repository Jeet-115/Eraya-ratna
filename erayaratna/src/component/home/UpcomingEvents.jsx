        ...
        {events.length > 0 ? (
          events.map((event) => (
            ...
            >
              ...
              <h3 className="text-lg font-semibold mb-2 text-center text-[#8A2C02]">{event.title}</h3>
              <p className="text-sm text-[#6C3D00] text-center mb-1">
                📅 {new Date(event.startTime).toLocaleDateString()}
              </p>
              <p className="text-sm text-[#6C3D00] text-center">📍 {event.location}</p>
            </motion.div>
          ))
        ) : (
          <p className="text-center text-gray-600 w-full italic">No upcoming events.</p>
        )}
      </div>
    </div>
